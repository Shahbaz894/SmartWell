...
class KhataService:
    ...
    def create_entry(self, user_id: str, data: dict):
        try:
            ...
            # 5. ✅ Remove conflicting keys before unpacking into KhataEntry
            data.pop("customer_id", None)   # set explicitly below
            data.pop("created_at", None)    # handled by DB server_default

            # 6. Save to Database
            entry = KhataEntry(
                id=str(uuid4()),
                customer_id=user_id,  # ✅ always from logged-in user
                **data
            )
            ...
            logger.error(f"Khata creation failed: {e}", exc_info=True)
