...
#             # ---- YAHAN FIX KAREIN ----
#             # Payload dictionary se 'trigger_type' nikalen (ESP32 jo bhej raha hai)
#             incoming_trigger = payload.get("trigger_type")
            
#             # Ab hum incoming_trigger pass karenge
#             trigger = self._determine_trigger_type(db, device.id, current_trigger=incoming_trigger)
...
# ... (baaki imports)
    ...
    def _determine_trigger_type(self, db: Session, device_id: Any, current_trigger: Optional[TriggerType] = None) -> TriggerType:
        if current_trigger in [TriggerType.APP, TriggerType.SCHEDULE, TriggerType.TIMER]:
            return current_trigger
        ...
        if isinstance(device_id, str):
            device = self._get_device_by_uid_or_id(db, device_id)
            if device:
                resolved_uuid = device.id
        else:
            resolved_uuid = device_id

        if resolved_uuid:
            # FIX: Cast column to String for comparison with UUID string
            active_timer = db.query(MotorTimer).filter(
                cast(MotorTimer.device_id, String) == str(resolved_uuid), 
                MotorTimer.is_running == True
            ).first()
    ...
    def create_telemetry_from_mqtt(self, db: Session, device_uid: str, payload: dict) -> MotorTelemetry:
        try:
            ...
            try:
                if raw_trigger:
                    incoming_trigger = TriggerType(raw_trigger) 
            except ValueError:
