...
class ScheduleService:
    ...
    # ✅ CREATE / UPDATE (ONE schedule per device)
    def create_schedule(self, device_id: str, schedule_in: ScheduleCreate):
        try:
            schedule = Schedule(
                id=str(uuid.uuid4()),
            ...
            return self.repo.create_or_update(schedule)

        except Exception as e:
            logger.error(f"CREATE_SCHEDULE_ERROR: {str(e)}")
            raise AppException("Failed to create schedule")
    ...
    def check_and_run(self):
        ...
        try:
            ...
            for schedule in schedules:
                ...
                for slot in slots:
                    try:
                        ...
                        logger.error(f"SLOT_ERROR: {str(e)}")

                # 🔥 MOTOR CONTROL
                is_running = self.motor_service.is_motor_running(schedule.device_id)

                if should_run and not is_running:
                    logger.info(f"START MOTOR {schedule.device_id}")
                    self.motor_service.start_motor(schedule.device_id, "schedule")
                    ...
                    if running_log and running_log.trigger_type == "schedule":
                        logger.info(f"STOP MOTOR {schedule.device_id}")
                        self.motor_service.stop_motor(schedule.device_id)
            ...
            logger.error(f"CRON_ERROR: {str(e)}")