...
class UserRepository:
    ...
    def create_user(self, user: User):
        try:
            ...
            self.db.rollback()
            logger.error("Failed to create user %s: %s", user.email, str(e))
            raise AppException(f"Database error: failed to create user {user.email}")
    ...
    def get_user_by_email(self, email: str):
        try:
            ...
            logger.error("Failed to fetch user by email %s: %s", email, str(e))
            raise AppException(f"Database error: failed to fetch user by email {email}")
